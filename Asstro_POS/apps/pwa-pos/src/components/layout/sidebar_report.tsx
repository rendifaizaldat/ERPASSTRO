...
import {
  ...
  Tag,
  ShieldAlert,
  CheckCircle, // ikon untuk tombol selesai
} from "lucide-react";
...
export const SidebarReport = ({
  ...
  const [isClosing, setIsClosing] = useState(false); // state loading untuk closing
  ...
  // Fungsi untuk menyelesaikan transaksi hari ini (closing shift)
  const handleCompleteShift = async () => {
    const confirmClose = window.confirm(
      "Apakah Anda yakin ingin menyelesaikan transaksi hari ini?\n\nSetelah dikunci, laporan tidak dapat diubah dan shift akan dianggap selesai.",
    );
    ...
    try {
      ...
      // Ganti dengan implementasi API Anda, misal:
      // await closeShift({ date: new Date(), reportData: modData });
      // atau dispatch action dari usePos jika tersedia.

      // Contoh dummy: simulasikan delay
      await new Promise((resolve) => setTimeout(resolve, 1000));

      // Tampilkan ringkasan akhir
      alert(
        `Shift selesai!\n\nTotal Omset: Rp ${modData.totalGross.toLocaleString()}\nTotal Transaksi: ${modData.totalTrx}\nKas Akhir: Rp ${modData.totalNet - modData.pettyCashOut - modData.totalRefund - modData.totalVoid}`,
      );

      // Opsional: tutup sidebar atau reset state (disesuaikan kebutuhan)
      // onClose();
    } catch (error) {
      alert("Gagal menyelesaikan shift. Silakan coba lagi.");
    } finally {
      setIsClosing(false);
    }
  };
        ...
        <div className="flex-1 overflow-auto p-5 bg-slate-100 scrollbar-thin print:bg-white print:p-0">
          ...
          {activeTab === "MOD" && (
            <div className="space-y-4 max-w-2xl mx-auto animate-fade-in print:max-w-none text-slate-800">
              ...
              {/* Tombol Selesaikan Transaksi Hari Ini */}
              <div className="flex justify-end print:hidden">
                <button
                  onClick={handleCompleteShift}
                  disabled={isClosing}
                  className={`px-4 py-2 rounded-xl font-black text-xs uppercase flex items-center gap-2 shadow-md transition-all cursor-pointer ${
                    isClosing
                ...
                >
                  <CheckCircle size={16} />
                  {isClosing ? "Memproses..." : "Selesaikan Transaksi Hari Ini"}
                </button>
              </div>
