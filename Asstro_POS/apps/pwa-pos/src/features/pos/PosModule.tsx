...
export const PosModule: React.FC<PosModuleProps> = ({
  ...
  const handleFinalCheckoutSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // PERBAIKAN BUG: Semua meja aktif (baik Takeaway maupun Meja Virtual Split Bill) harus dihancurkan setelah lunas
      if (selectedTable) {
  ...
  return (
    <div className="w-full h-full flex flex-row overflow-hidden bg-[#F8FAFC]">
      ...
      {/* PERBAIKAN BUG: Inject props yang benar (tableLabel, operatorName, onConfirmSplit) */}
      <SplitBillModal
      ...
      {/* PERBAIKAN BUG: Inject props yang benar (dbTables, tableLabel, operatorName, onConfirmMove) */}
      <MoveOrderModal
