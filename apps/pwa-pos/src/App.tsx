...
// [REMOVED] import { SecurityEnforcer } from "./components/shared/SecurityEnforcer";
...
function AppContent() {
  ...
  useEffect(() => {
    const handleTestPrint = (e: any) => {
      printerService.printReceipt(
        { test: true, message: "TEST PRINT SUCCESS" },
        { printerSettings: e.detail.config },
      );
      showToast(`Test Print dikirim ke ${e.detail.target}`, "SUCCESS");
    };

    const handleThermalPrint = (e: any) => {
      printerService.printReceipt(e.detail.payload, {
        printerSettings: e.detail.settings,
      });
    };
  ...
  // ✅ SecurityEnforcer dihapus, komponen dirender langsung
  return (
    <>
      ...
      {/* Reauth Modal tetap digunakan tanpa SecurityEnforcer */}
      <ReauthModal
