...
export const SetupWizard: React.FC = () => {
  ...
  const runRecoveryProcess = async (
    ...
    try {
      ...
      let chunkIndex = parseInt(
        localStorage.getItem("ASSTRO_HYDRATION_CHECKPOINT") || "1",
        10,
      );
      ...
      while (retries < MAX_RETRIES) {
        try {
          // [FIX]: Penulisan URL Hydrate yang Aman
          const baseUrl = `${import.meta.env.VITE_API_URL || "http://localhost:4000"}/api/sync/hydrate`;
          const queryParams = new URLSearchParams({
            chunk: chunkIndex.toString(),
          });
          if (replaceId) queryParams.append("replaceDeviceId", replaceId);

          const hydrateUrl = `${baseUrl}?${queryParams.toString()}`;

          syncResponse = await fetch(hydrateUrl, {
            method: "GET",
            headers: {
          ...
          if (retries >= MAX_RETRIES) {
            addLog(
              `[FATAL] Hydration gagal setelah ${MAX_RETRIES} percobaan. Klik Reset Provisioning untuk mengulang.`,
            );
            throw err;
          }
          addLog(
            `[RETRY] Percobaan ke-${retries} gagal. Retrying in ${backoffDelay}ms...`,
          );
