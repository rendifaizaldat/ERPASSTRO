...
export const LoginScreen: React.FC = () => {
  ...
  return (
    <div className="h-screen w-full flex flex-col items-center justify-center bg-slate-950 text-slate-100 p-6 font-sans select-none relative">
      ...
      {showFloatModal && (
        <div className="fixed inset-0 bg-slate-950/80 z-100 backdrop-blur-sm flex items-center justify-center p-4 animate-fade transition-all duration-300 ease-in-out">
          <div className="w-full max-w-md bg-white text-slate-900 rounded-[2.5rem] shadow-2xl p-6 border border-slate-200 flex flex-col animate-fade-in max-h-[80vh] min-h-[60vh] overflow-y-auto">
            ...
            <form onSubmit={handleConfirmOpeningShift} className="space-y-4">
              {/* FORM INPUT - langsung di bawah header */}
              <div>
              ...
              {/* TEKS SAMBUTAN - dipindahkan ke bawah form input */}
              ...
              {/* DESKRIPSI PANJANG - tetap di bawah teks sambutan */}
                ...
                <div className="flex items-start gap-2">
                  ...
                  <p className="text-[10px] font-medium text-slate-600 leading-relaxed">
                    <span className="font-black text-slate-700">
                      Apa itu uang modal kasir?
                    </span>{" "}
                    Uang modal kasir adalah sejumlah uang tunai yang disiapkan
                    pada awal shift operasional sebagai saldo awal laci kas
                    (cash drawer). Dana ini digunakan untuk memberikan uang
                    kembalian kepada pelanggan selama transaksi berlangsung.
                    Uang modal bukan merupakan pendapatan restoran dan harus
                    tetap utuh hingga akhir shift, kecuali terdapat penambahan
                    atau pengurangan yang telah dicatat melalui prosedur resmi.
                    Pada saat penutupan kasir, uang modal dihitung kembali dan
                    dipisahkan dari hasil penjualan untuk memastikan
                    keseimbangan kas dan memudahkan proses rekonsiliasi.
                  </p>
                </div>
              </div>
              ...
              <button
                type="submit"
                className="w-full bg-orange-600 hover:bg-slate-900 text-white py-5 rounded-2xl font-black text-xs uppercase tracking-widest transition-all shadow-xl active:scale-95 cursor-pointer mt-4"
              >
                Buka Shift Sekarang
              </button>
            </form>
      ...
      {showHandoverModal && handoverData && (
        <div className="fixed inset-0 bg-slate-950/80 z-100 backdrop-blur-sm flex items-center justify-center p-4 animate-fade transition-all duration-300 ease-in-out">
          <div className="w-full max-w-md bg-white text-slate-900 rounded-[2.5rem] shadow-2xl p-6 border border-slate-200 flex flex-col animate-fade-in max-h-[80vh] min-h-[60vh] overflow-y-auto">
            ...
            {/* Catatan singkat sebagai pengisi ruang */}
            <div className="bg-blue-50/70 border border-blue-200/60 rounded-2xl p-3 mb-4">
              <p className="text-[10px] font-medium text-slate-600 leading-relaxed">
                <span className="font-black text-blue-700">Catatan:</span>{" "}
                Dengan menerima operan, Anda menyetujui bahwa data fisik dan
                laporan di atas telah diverifikasi. Uang modal akan otomatis
                terbawa ke shift Anda.
              </p>
            </div>
