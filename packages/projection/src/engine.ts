  ...
  private handleEvent(event: LedgerEvent) {
    ...
    switch (event.type) {
      ...
      case "ORDER_UPDATED": {
        ...
        const orderId = payload.orderId;
        const updatedBill = this.calculateOrderBill(payload.items || []);

        // 1. Tangani Skenario Pindah Meja: Bersihkan meja lama jika order ini pindah
        ...
        // Update referensi dasar order
        const existingOrderIndex = this.transactions.findIndex(
          (tx) => tx._internalId === orderId && tx._type === "ORDER",
        );
        if (existingOrderIndex >= 0) {
          this.transactions[existingOrderIndex] = {
            ...this.transactions[existingOrderIndex],
            ...payload,
          };
        } else {
      ...
      case "INVOICE_CREATED": {
        ...
        // Cari referensi order untuk mendapatkan data menu (items)
        const linkedOrder = this.transactions.find(
          (tx) => tx._internalId === orderId && tx._type === "ORDER",
        );

        this.transactions.push({
          ...payload,
          _internalId: payload.invoiceId,
          _type: "INVOICE",
          items: linkedOrder ? linkedOrder.items : [], // Penting untuk Laporan PLU/Kategori
          tableLabel: linkedOrder ? linkedOrder.tableLabel : "UNKNOWN",
      ...
      case "PAYMENT_RECEIVED": {
        ...
        // Cari dan update invoice untuk Laporan Keuangan Akhir
        const invoiceIndex = this.transactions.findIndex(
          (tx) =>
            tx._internalId === payload.invoiceId && tx._type === "INVOICE",
        );
        if (invoiceIndex >= 0) {
          this.transactions[invoiceIndex] = {
            ...this.transactions[invoiceIndex],
            ...payload, // Menggabungkan informasi method pembayaran
            status: "PAID",
          };
        }
        break;
      ...
      case "SALE_CREATED": {
        ...
        this.transactions.push({ ...payload, status: "PAID" });
        break;
      }
      ...
      case "SHIFT_OPENED": {
        ...
        this.shiftInitialCash = Number(
          payload.startingCash || payload.initial_cash || 0,
        );
        this.transactions = [];
        this.auditLogs = [];
      ...
      case "PETTY_CASH_RESOLVED": {
        const payload = event.payload as any;
        const index = this.pettyCashTransactions.findIndex(
          (p: any) => p.petty_cash_id === payload.petty_cash_id,
        );
        if (index >= 0) {
          this.pettyCashTransactions[index] = {
            ...this.pettyCashTransactions[index],
            status: "COMPLETED",
            ...payload,
          };
  ...
  public getState() {
    ...
    if (!this.reportData || !this.reconData || !this.cashData) {
      ...
      completedTransactions.forEach((tx: any) => {
        ...
        paymentSales[method] =
          (paymentSales[method] || 0) + (tx.grand_total || tx.amountPaid || 0);
        ...
        (tx.items || []).forEach((item: any) => {
          ...
          if (activeQty > 0) {
            ...
            catSales[catName].total +=
              (item.price || item.basePriceSnapshot || 0) * activeQty;
          }
        });
      });
      ...
      this.auditLogs.forEach((a: any) => {
        ...
        if (
          a.eventType === "PAYMENT_REFUNDED" ||
          a.eventType === "ORDER_REFUNDED"
        ) {
      ...
      const systemCash =
        this.shiftInitialCash +
        cashSales -
        pettyCashOut -
        (totalRefund - totalNonCashRefund);
      ...
      completedTransactions.forEach((tx: any) => {
        (tx.items || []).forEach((item: any) => {
          ...
          if (activeQty > 0) {
            ...
            pluMap[itemName].total +=
              (item.price || item.basePriceSnapshot || 0) * activeQty;
          }
        });
      });

      const pluData = Object.entries(pluMap).sort(
        (a, b) => b[1].qty - a[1].qty,
      );
      ...
      const recon = {
        ...
        systemNonCash,
        activeTables,
        voidRefundCount: this.auditLogs.filter(
          (log) =>
